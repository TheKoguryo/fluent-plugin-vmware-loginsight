...
RUN buildDeps="\
    ...
    #
    # Install fluentd
    && gem install --norc --no-document fluentd -v 1.10.3 \
    && mkdir -p /fluentd/etc /fluentd/plugins \
    #
    # Install Log Insight plugin
    && gem install --norc --no-document -v 0.1.9 fluent-plugin-vmware-loginsight \
    #
    && gem install --norc --no-document -v 1.0.0 fluent-plugin-multi-format-parser \
    && gem install --norc --no-document -v 0.0.13 fluent-plugin-detect-exceptions \ 
    && gem install --norc --no-document -v 2.4.0 fluent-plugin-concat \
    && gem install --norc --no-document -v 2.4.5 fluent-plugin-kubernetes_metadata_filter \
    && cp /usr/lib/ruby/gems/2.5.0/gems/fluent-plugin-vmware-loginsight-0.1.9/lib/fluent/plugin/out_vmware_loginsight.rb /fluentd/plugins \
    && cp /usr/lib/ruby/gems/2.5.0/gems/fluent-plugin-multi-format-parser-1.0.0/lib/fluent/plugin/parser_multi_format.rb /fluentd/plugins \
    && cp /usr/lib/ruby/gems/2.5.0/gems/fluent-plugin-detect-exceptions-0.0.13/lib/fluent/plugin/out_detect_exceptions.rb /fluentd/plugins \
    && cp /usr/lib/ruby/gems/2.5.0/gems/fluent-plugin-detect-exceptions-0.0.13/lib/fluent/plugin/exception_detector.rb /fluentd/plugins \
    && cp /usr/lib/ruby/gems/2.5.0/gems/fluent-plugin-concat-2.4.0/lib/fluent/plugin/* /fluentd/plugins \
    && cp /usr/lib/ruby/gems/2.5.0/gems/fluent-plugin-kubernetes_metadata_filter-2.4.5/lib/fluent/plugin/* /fluentd/plugins \
    #
